...
object Surov3CoreSim extends App {
  Config.sim.withWave.compile(SimDUT(SurovConfig())).doSim { dut =>
    ...
    for (i <- 0 to 10000) {
      ...
      if (s"${dut.core.pipeline.opcode.toEnum}" == "Sys") {
        println(s"ecall Called gp: ${dut.rf.getBigInt(3)} i = $i")
        simSuccess()        
      }
    }
    for (i <- 0 to 3) {
    ...
    simFailure("Didn't finish after 10000 steps")
